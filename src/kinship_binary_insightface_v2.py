...
def mine_hard_negatives(embeddings1, embeddings2, labels, k=0.5):
    """
    Mine hard negatives from batch using cosine similarity.
    Args:
        embeddings1, embeddings2: Normalized embeddings tensors
        labels: Binary labels tensor
        k: Float proportion of hardest negatives to keep
    Returns:
        hard_mask: Boolean mask for hard negatives
    """
    with torch.no_grad():
        ...
        # Select threshold for hard negatives
        k = max(1, int(k * len(neg_similarities)))
        threshold = torch.topk(neg_similarities, k).values[-1]
        
        # Create final mask (keep all positives and hard negatives)
        hard_mask = (labels == 1) | ((similarities >= threshold) & (labels == 0))
        
        return hard_mask

class HardNegativeContrastiveLoss(nn.Module):
    def __init__(self, margin=0.5, hard_ratio=0.5):
        super().__init__()
        self.margin = margin
    ...
    def forward(self, embeddings1, embeddings2, labels):
        ...
        # Mine hard negatives
        hard_mask = mine_hard_negatives(embeddings1, embeddings2, labels, self.hard_ratio)
        
        # Calculate similarities for selected pairs
        similarity = F.cosine_similarity(embeddings1[hard_mask], embeddings2[hard_mask])
        labels = labels[hard_mask].float()
        ...
        if transform is None:
            if is_training:
                self.transform = transforms.Compose([
                    ...
                    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2),  # Increased intensity
                    transforms.RandomRotation(15),  # Added rotation
                    transforms.Resize((112, 112)),
                    transforms.ToTensor(),
                    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
                    transforms.RandomErasing(p=0.2)  # Increased probability
                ])
            else:
                self.transform = transforms.Compose([
                    transforms.ToPILImage(),
...
def train_model(model, train_loader, val_loader, test_loader, config):
    ...
    for epoch in range(config['epochs']):
        ...
        if val_metrics['accuracy'] > best_val_metrics['accuracy']:
            ...
            torch.save(checkpoint, 'checkpoints_v4/best_model_hard_negative.pth')
...
def main():
    ...
    config = {
        'batch_size': 256,  # Increased for better hard negative mining
        'lr_backbone': 1e-5,
        'epochs': 15,
        'weight_decay': 0.01,
        'patience': 7,
        ...
        'margin': 0.5,
        'hard_ratio': 0.5  # Ratio of hard negatives to keep
    }
    ...
    if args.test_only:
        ...
        best_checkpoint = torch.load('checkpoints_v4/best_model_hard_negative.pth')
        ...
        comprehensive_results = {
            ...
            'config': config,
            'model_info': {
                'checkpoint_path': 'checkpoints_v4/best_model_hard_negative.pth',
                'epoch': best_checkpoint['epoch'],
                'timestamp': time.strftime('%Y%m%d-%H%M%S')
            }
        }
        
        with open('checkpoints_v4/comprehensive_test_results_hard_negative.json', 'w') as f:
            json.dump(comprehensive_results, f, indent=4)
        
        print("\nComprehensive results saved to: checkpoints_v4/comprehensive_test_results_hard_negative.json")
        return

    # Initialize wandb for training
    run = wandb.init(
        project="kinship-verification-hard-negative",
        config=config,
        tags=['contrastive', 'hard-negative-mining']
    )

    if args.continue_training:
        print("\nContinuing training from last checkpoint...")
        checkpoint = torch.load('checkpoints_v4/best_model_hard_negative.pth')
    ...
    with open('checkpoints_v4/final_results_hard_negative.json', 'w') as f:
